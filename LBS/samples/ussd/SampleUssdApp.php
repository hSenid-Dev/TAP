...
//logFile("[ content=$content, addressTest=$address]");



function loadUssdSender($sessionId, $responseMessage)
{
    $password = "12d6a01921db7d414bc5514acd49bd82";
  //  $destinationAddress = "tel:AZ110hnY8eJS5Cpmh+Qz9eosg+n4y27OX4H5AxZgXWLYuNvnDV0ctxfdI/wnlY0yPhO6V";
...
//        $sender = new MtUssdSender("http://localhost:7000/ussd/send/");   // Application ussd-mt sending http url
	  ...
	  $sender = new MtUssdSender("https://api.dialog.lk/ussd/send");
        $response = $sender->ussd($applicationId, $password, $version, $responseMessage,$chargingAmount
            $sessionId, $ussdOperation, $destinationAddress, $encoding);
        return $response;
